...
public class HW6 {
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> df = d.filter(f ->f.get(ORIGIN_CITY_NAME).equals("Seattle, WA"));
    JavaRDD<Object> rows= df.map(row->row.get(DEST_CITY_NAME));
    JavaRDD<Row> r = rows.map(RowFactory::create).distinct();
    return r;
  }
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
    JavaRDD<Row> nC = d.filter(f -> f.getInt(CANCELLED) !=1);
    JavaPairRDD<Tuple2<String,Integer>,Integer> r = nC.mapToPair(t->new Tuple2<Tuple2<String,Integer>,Integer>(new Tuple2<String,Integer>(t.getAs(ORIGIN_CITY_NAME),t.getInt(MONTH)),1)).reduceByKey((a,b)->a+b);
    return r;
  }
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
    JavaPairRDD<String, String> cityPair = d.mapToPair(t->new Tuple2<String, String>(t.getAs(ORIGIN_CITY_NAME),t.getAs(DEST_CITY_NAME))).distinct();
    JavaPairRDD<String, Integer> oriCity = cityPair.mapToPair(t->new Tuple2<String,Integer>(t._1,1)).reduceByKey((a,b)->a+b);
    Tuple2<String,Integer> maxCity = oriCity.max(new valueComparator());
    String maxCityName = maxCity._1;
    JavaRDD<Row> maxCityfilter = d.filter(f ->f.getAs(ORIGIN_CITY_NAME).equals(maxCityName));
    Tuple2<String, Integer> flightCount = maxCityfilter.mapToPair(t->new Tuple2<String, Integer>(t.getAs(ORIGIN_CITY_NAME),1)).reduceByKey((a,b)->a+b).first();  
    return flightCount;
  }
  private static class valueComparator implements Serializable,Comparator<Tuple2<String, Integer>> {
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
    JavaRDD<Row> allF = d.filter(f ->f.get(DEP_DELAY) != null);
    //<cityname, delay, count>
    JavaPairRDD<String,Tuple2<Integer,Integer>> delay = allF.mapToPair(t->new Tuple2<String,Tuple2<Integer,Integer>>(t.getAs(ORIGIN_CITY_NAME),new Tuple2<Integer,Integer>(t.getAs(DEP_DELAY),1)));
    JavaPairRDD<String,Tuple2<Integer,Integer>> total_delay = delay.reduceByKey((t1,t2) ->  new Tuple2<Integer, Integer>(t1._1 + t2._1, t1._2 + t2._2));
    JavaPairRDD<String, Double> ave_delay = total_delay.mapToPair(t->new Tuple2<String, Double>(t._1,t._2._1*1.0/t._2._2));
    return ave_delay;
  }
